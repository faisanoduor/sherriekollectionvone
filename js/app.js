...
function waLink(order) { const text = `Hello Sherie Kollections,\nI would like to place order ${order.id}.\nItems:\n${order.items.map(i => `- ${i.name} x${i.qty}${i.size ? " (" + i.size + ")" : ""}`).join("\n")}\nTotal: ${money(order.total)}\nName: ${order.customer.name}\nPhone: ${order.customer.phone}`; return `https://wa.me/${CONFIG.whatsapp}?text=${encodeURIComponent(text)}` }
